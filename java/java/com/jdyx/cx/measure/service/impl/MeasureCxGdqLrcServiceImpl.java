...
import com.jdyx.cx.measure.service.MeasureCxGdqLrcService;
...
@Service
public class MeasureCxGdqLrcServiceImpl implements MeasureCxGdqLrcService {
  ...
  @Resource
  private IntTPpJlIacfFolNewMapper tPpJlIacfFolMapper;
...
//    piShouldRecPayOffMonMapper.setPiShouldRecPayOffMonUnitId(DateUtils.endMonth(valMonth, DateUtils.YYYY_MM_DD));
    ...
    //上年未到期明细的总获取费用、跟单费用
    Map<String, Map<String, BigDecimal>> lastMeasureCxUnexpired = getLastMeasureCxUnexpired(valMethod, DateUtils.lastEndYear(valMonth, DateUtils.YYYYMM),
    ...
    //生成2412合同基础信息表
    intTPpJlContractNewMapper.delete(new LambdaQueryWrapper<IntTPpJlContractNew>()
      .eq(IntTPpJlContractNew::getValMonth, valMonth));
    intTPpJlContractNewMapper.createTPpJlContactByGdq24();
    //生成2412获取费用跟单
    tPpJlIacfFolMapper.delete(
      new LambdaQueryWrapper<IntTPpJlIacfFolNew>().eq(IntTPpJlIacfFolNew::getValMonth, valMonth));
    tPpJlIacfFolMapper.createIacfFolGdq();
    //生成2412获取费用非跟单
    tPpJlIacfUnfolNewMapper.delete(
      new LambdaQueryWrapper<IntTPpJlIacfUnfolNew>().eq(IntTPpJlIacfUnfolNew::getValMonth, valMonth));
    tPpJlIacfUnfolNewMapper.createTPpJlIacfUnfolByGdq24();
    //生成2412计量源数据
    ...
    measureCfBasicDataNewMapper.createBasicDataByGdq();
    //生成2412失效保单数据
...
//    long processedTotal = 0; // 本次已处理数量
    ...
    while (true) {
      ...
      lqw.select(MeasureCfBasicDataNew::getId, MeasureCfBasicDataNew::getPolicyNo,MeasureCfBasicDataNew::getCertiNo, MeasureCfBasicDataNew::getGroupId,
          ...
          MeasureCfBasicDataNew::getEndDate, MeasureCfBasicDataNew::getTerm, MeasureCfBasicDataNew::getIacfFolCny,MeasureCfBasicDataNew::getIacfUnfolCny,
          MeasureCfBasicDataNew::getUnitId,MeasureCfBasicDataNew::getComCode,MeasureCfBasicDataNew::getBusinessNature,MeasureCfBasicDataNew::getCarKindCode,MeasureCfBasicDataNew::getUseNatureCode)
        .eq(MeasureCfBasicDataNew::getValMonth, valMonth)
        .eq(MeasureCfBasicDataNew::getValMethod, valMethod)
...
//    });
  ...
  private MeasureCxUnexpired calculateLrcWithMonthlyRolling(MeasureCfBasicDataNew contract, String valMonth ,String valMethod) {
    try {
      ...
      int servedDays = 0;
      //上期未到期明细的总获取费用、跟单费用
      Map<String, BigDecimal> lastMeasureCxUnexpired = lastMeasureCxUnexpiredCache.getOrDefault(contract.getUnitId(), new HashMap<>());
      ...
      // 获取费用=（跟单+非跟单）
      BigDecimal iacfCashflow = contract.getIacfFolCny().add(contract.getIacfUnfolCny()).add(lastTotalIacfAmt);
      //TODO 前海特有，跟单获取费用部分
...
//      cashFlowList.add(arr[i].setScale(2, RoundingMode.HALF_UP));
   ...
   */
  public static Map<String, Integer> calculatePayments(String iniConfirm, String start, String valMonth, String end, Integer term) {
    ...
    if(iniConfirmDate.isAfter(parseEnd)){
      result.put(iniConfirmDate.format(YYYYMM_FORMATTER),term);
      return result;
    }
    //评估期和保险止期取早
