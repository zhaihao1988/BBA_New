...
import com.jdyx.cx.measure.service.MeasureCxCsLrcService;
...
@Service
public class MeasureCxCsLrcServiceImpl implements MeasureCxCsLrcService {

  @Resource
  private MeasureCfBasicDataMapper measureCfBasicDataMapper;
  @Resource
  ...
  private final Map<String, Map<String, BigDecimal>> lastMeasureCxUnexpiredCache = new ConcurrentHashMap<>();
...
//  private ThreadPoolExecutor threadPoolExecutor;
  ...
  private void preloadCacheData(String valMethod,String valMonth) throws ParseException {
    ...
    payOffMonQuery.select(PiShouldRecPayOffMon::getPolicyNo, PiShouldRecPayOffMon::getCertiNo, PiShouldRecPayOffMon::getCancelDate, PiShouldRecPayOffMon::getCancelAmount)
      .le(PiShouldRecPayOffMon::getStatDate, DateUtils.endMonth(DateUtils.parseDate(valMonth)))
    ...
    //上期未到期明细的总获取费用、跟单费用
    Map<String, Map<String, BigDecimal>> lastMeasureCxUnexpired = getLastMeasureCxUnexpired(valMethod, DateUtils.lastEndYear(valMonth, DateUtils.YYYYMM), MeasureCxUnexpired::getPolicyNo,MeasureCxUnexpired::getCertiNo,MeasureCxUnexpired::getTotalIacfAmt, MeasureCxUnexpired::getTotalIacfDirectAmt);
    ...
    lastMeasureCxUnexpiredCache.putAll(lastMeasureCxUnexpired);
  ...
  private void processDataWithCursorPagination(String valMethod, String valMonth)
    ...
    // 1. 获取总数据量
    LambdaQueryWrapper<MeasureCfBasicData> countQuery = Wrappers.lambdaQuery();
    countQuery.eq(MeasureCfBasicData::getValMethod, valMethod)
      .eq(MeasureCfBasicData::getValMonth, valMonth);
//    long selectCount = measureCfBasicDataMapper.selectCount(countQuery);
//
//    // 2. 计算批次数量
...
//    long processedTotal = 0; // 本次已处理数量
    ...
    while (true) {
      Long startTime = System.currentTimeMillis();
      // 使用游标方式分页查询
      LambdaQueryWrapper<MeasureCfBasicData> lqw = new LambdaQueryWrapper<>();
      lqw.select(MeasureCfBasicData::getId, MeasureCfBasicData::getPolicyNo,MeasureCfBasicData::getCertiNo, MeasureCfBasicData::getGroupId,
          MeasureCfBasicData::getClassCode, MeasureCfBasicData::getPremiumCny,MeasureCfBasicData::getIniConfirm, MeasureCfBasicData::getStartDate,
          MeasureCfBasicData::getEndDate, MeasureCfBasicData::getTerm, MeasureCfBasicData::getAccPrem, MeasureCfBasicData::getIacfUnfol,MeasureCfBasicData::getIacfFol)
        .eq(MeasureCfBasicData::getValMonth, valMonth)
        .eq(MeasureCfBasicData::getValMethod, valMethod)
        .gt(MeasureCfBasicData::getId, maxId)
        .orderByAsc(MeasureCfBasicData::getId)
        .last("LIMIT " + BATCH_SIZE);
      List<MeasureCfBasicData> records = measureCfBasicDataMapper.selectList(lqw);
      if (records.isEmpty()) {
        break;
      }
...
//    log.debug("数据处理完成,数量:{}",processedTotal);
   ...
   * 异步处理批次数据
   */
  private void processBatchAsync(List<MeasureCfBasicData> batchData, String valMonth,String valMethod) {

    // 将参数声明为final，避免lambda表达式中的变量引用问题
    final String finalValMonth = valMonth;
    final String finalValMethod = valMethod;
    final List<MeasureCfBasicData> finalBatchData = batchData;
...
//    });
   ...
   * 过渡期从保险责任起期滚动计量到评估时点
   */
  private MeasureCxUnexpired calculateLrcWithMonthlyRolling(MeasureCfBasicData contract, String valMonth ,String valMethod) {
    try {
      // 1. 生成签单日到评估期中，每个月有效的天数（保单起期后才有效）
      Map<String, Integer> periodMap = calculatePayments(contract.getIniConfirm(), contract.getStartDate(), valMonth, contract.getEndDate(),contract.getTerm());
      ...
      if (iniConfirmAssumption == null) {
        log.error("初始确认日{}该险类{}下的精算假设配置表不存在", contract.getIniConfirm(), contract.getClassCode());
      }
      //获取评估时点该险类下的精算假设配置表
      ...
      int servedDays = 0;
      //上期未到期明细的总获取费用、跟单费用
      Map<String, BigDecimal> lastMeasureCxUnexpired = lastMeasureCxUnexpiredCache.getOrDefault(contract.getPolicyNo() + "_" + contract.getCertiNo(), new HashMap<>());
      ...
      // 获取费用=（跟单+非跟单）
      BigDecimal iacfCashflow = contract.getIacfUnfol().add(contract.getIacfFol()).add(lastTotalIacfAmt);
      //TODO 前海特有，跟单获取费用部分
      BigDecimal iacfCashflowDirect = contract.getIacfUnfol().add(lastTotalIacfDirectAmt);
      //下标记录记录当前滚动的月份数
      int x = 0;
      //I17初始确认日到止期经过的月份数
      ...
      // 从缓存中获取该保单的实收保费数据
      String key = StringUtils.joinWith("_", contract.getPolicyNo(), contract.getCertiNo());
      ...
      for (Map.Entry<String, Integer> entry : periodMap.entrySet()) {
        ...
        //TODO 获取费用假设在第一期就全部支出
        if (x == 1) {
          iacfCashflowCurrent = iacfCashflow;
          iacfCashflowCurrentDirect = iacfCashflowDirect;
          //获取费用利息=预期支付获取费用总额 * 利率
          iacfCashflowIfie = iacfCashflow.multiply(disRate).multiply(IFIE_RATIO).setScale(10, RoundingMode.HALF_UP);
          //TODO 前海特有 跟单获取费用利息=预期支付获取费用跟单 * 利率
          iacfCashflowDirectIfie = iacfCashflowDirect.multiply(disRate).multiply(IFIE_RATIO).setScale(10, RoundingMode.HALF_UP);
...
//        entry.getKey(),premiumCashflow,currentPremiums,iacfCashflowCurrent,currentIacf,iacfCashflowCurrentDirect,currentIacfDirect,currentIfie,currentIfieDirect,openingBalance,openingBalanceDirect,closingBalance,closingBalanceDirect,futurePremiums);
      ...
      BigDecimal expiredPremium = payOffMonthData.entrySet().stream()
        .filter(e -> e.getKey() != null && e.getKey()
          .compareTo(DateUtils.parseDateToStr(DateUtils.YYYYMM,DateUtils.parseDate(contract.getEndDate()))) > 0)
        .map(Map.Entry::getValue)
        .reduce(BigDecimal.ZERO, BigDecimal::add);
      closingBalance = closingBalance.add(expiredPremium);
      closingBalanceDirect = closingBalanceDirect.add(expiredPremium);

      //累计实收保费加上止期到评估期这段期间收到的保费
      cumulativeReceivedPremiums = cumulativeReceivedPremiums.add(expiredPremium);
      ...
      //保障期限
      measureCxUnexpired.setTerm(contract.getTerm());
      //签单保费
   ...
   * @return
   */
  private BigDecimal getPvLoss(BigDecimal amt,String classCode,int n,String valMonth,List<BigDecimal> cashFlowList,MeasureCfBasicData contract) {
...
//      cashFlowList.add(arr[i].setScale(2, RoundingMode.HALF_UP));
   ...
   */
  public static Map<String, Integer> calculatePayments(String iniConfirm, String start, String valMonth, String end, Long trem) {
    ...
    //如果签单日晚于止期，则返回签单日所在月份
    if(iniConfirmDate.isAfter(parseEnd)){
      result.put(iniConfirmDate.format(YYYYMM_FORMATTER),trem.intValue());
      return result;
    }
    //评估期和保险止期取早
  ...
  public Map<String, Map<String, BigDecimal>> getLastMeasureCxUnexpired(String valMethod, String valMonth, SFunction<MeasureCxUnexpired, ?>... selectVar) {
      ...
      lqw = StringUtils.isNotEmpty(selectVar) ?
        lqw
          ...
          //2.评估方法
          .eq(sqlFunctionUtil.getParamSql(MeasureCxUnexpired::getValMethod), valMethod) :
        lqw
          //1.评估时点
          .eq(sqlFunctionUtil.getParamSql(MeasureCxUnexpired::getValMonth), valMonth)
          //2.评估方法
      ...
      Map<String, Map<String, BigDecimal>> collect = infoList.stream().collect(Collectors.groupingBy(e -> e.getPolicyNo()+"_"+e.getCertiNo(),
        Collectors.collectingAndThen(
          Collectors.toList(),
          list -> list.stream()
  ...
  private void clearCache() {
    ...
    lastMeasureCxUnexpiredCache.clear();
    log.info("缓存清理完成");
  }
}
