...
import com.jdyx.cx.measure.service.MeasureCxLicUnsettledService;
...
@Service
public class MeasureCxLicUnsettledServiceImpl implements MeasureCxLicUnsettledService {
  @Resource
  ...
  public class MeasureContext {
    ...
    //上个评估月未决计量缓存对象
    private Map<String, MeasureCxUnsettled> lastUnsettledMap;
  }
  ...
  private MeasureContext preloadCacheData(String valMethod,String valMonth) {
    ...
    context.setLastUnsettledMap(unsettledMap);

    log.info("预加载完成 - 精算假设: {} 条, 赔付模式: {} 个险类,月度远期利率:{}条,上期未决结果:{}条",
      collect.size(), claimModelMap.size(),disrateMap.size(),unsettledMap.size());
    return context;
  }
  ...
  private void processDataWithCursorPagination(String valMethod, String valMonth,MeasureContext context) {
    ...
    while (true) {
      ...
      lqw.select(IntTPpJlUnsettledGroup::getId, IntTPpJlUnsettledGroup::getAccidentMonth,IntTPpJlUnsettledGroup::getClassCode,IntTPpJlUnsettledGroup::getCurrentFlag,
      ...
      // 处理批次数据
      processBatchAsync(records, valMonth,valMethod,context);
      // 更新游标
      maxId = records.get(records.size() - 1).getId();
    }
    //处理上期未决转已决的数据
    dealDecidedData(valMonth,valMethod,context);
  }

  /**
   * 异步处理批次数据
   */
  private void processBatchAsync(List<IntTPpJlUnsettledGroup> batchData, String valMonth,String valMethod,MeasureContext context) {
...
//        measureCxUnsettledMapper.insertBatch(batchResults);
  ...
  private void insertBatchWithJdbcTemplate(List<MeasureCxUnsettled> allResults) {
    ...
    List<String> columnNames = Arrays.asList(
      ...
      "pv_case_current_ra", "pv_ibnr_current_ra", "pv_ulae_current_ra",
      "pv_case_accident_ra", "pv_ibnr_accident_ra", "pv_ulae_accident_ra",
      "pv_last_case_amt_ra","pv_last_ibnr_amt_ra","pv_last_ulae_amt_ra",
      ...
      "create_time", "rein_type", "rein_system_code", "paid_claim_ifie_ra", "oci_change_ra"
    );
    ...
    jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
      ...
      public void setValues(PreparedStatement ps, int i) throws SQLException {
        ...
        ps.setBigDecimal(index++, item.getPvCaseCurrentRa());
        ps.setBigDecimal(index++, item.getPvIbnrCurrentRa());
        ps.setBigDecimal(index++, item.getPvUlaeCurrentRa());
        ps.setBigDecimal(index++, item.getPvCaseAccidentRa());
        ps.setBigDecimal(index++, item.getPvIbnrAccidentRa());
        ps.setBigDecimal(index++, item.getPvUlaeAccidentRa());
        ps.setBigDecimal(index++, item.getPvLastCaseAmtRa());
        ps.setBigDecimal(index++, item.getPvLastIbnrAmtRa());
        ps.setBigDecimal(index++, item.getPvLastUlaeAmtRa());
        ...
        ps.setBigDecimal(index++, item.getPaidClaimIfieRa());
        ps.setBigDecimal(index++, item.getOciChangeRa());
      }
   ...
   * 过渡期从保险责任起期滚动计量到评估时点
   */
  private MeasureCxUnsettled calculateLrcWithMonthlyRolling(IntTPpJlUnsettledGroup contract, String valMonth ,String valMethod,MeasureContext context) {
    //获取评估大类对应的精算假设
    Map<String, ConfMeasureActuarialAssumption> actuarialAssumptionMap = context.getAssumptionCache().get(valMonth);
    ...
    //上期未决结果
    MeasureCxUnsettled lastUnsettled = context.getLastUnsettledMap().getOrDefault(StringUtils.joinWith("_",DateUtils.lastEndMonth(valMonth),contract.getAccidentMonth(), contract.getUnitId()), new MeasureCxUnsettled());
    //出险年月的月度远期利率曲线
      Map<Integer, BigDecimal> accidentRateMap = context.getDisRateCache().getOrDefault(contract.getAccidentMonth(),new HashMap<>());
    //评估时点的月度远期利率曲线
    Map<Integer, BigDecimal> valMonthsRateMap = context.getDisRateCache().getOrDefault(valMonth,new HashMap<>());
    //当期险类对应的未决赔付模式进展因子
    BigDecimal[] claimFactorArr = context.getDiscountFactorCache().get(contract.getClassCode());
    ...
    //移除上期已经匹配到到数据
    if(!context.getLastUnsettledMap().isEmpty()){
      context.getLastUnsettledMap().remove(StringUtils.joinWith("_",DateUtils.lastEndMonth(valMonth),contract.getAccidentMonth(),contract.getUnitId()));
    }
    ...
    //本评估时点未决赔款现金流的本期末现值_RA（用事故时点利率曲线折现）
    measureCxUnsettled.setPvUlaeAccidentRa(pvUlaeAccident.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setPvIbnrAccidentRa(pvIbnrAccident.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setPvCaseAccidentRa(pvCaseAccident.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    ...
    //本评估时点未决赔款现金流的本期末现值_RA（用当期利率曲线折现）
    measureCxUnsettled.setPvUlaeCurrentRa(pvUlaeCurrent.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setPvIbnrCurrentRa(pvIbnrCurrent.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setPvCaseCurrentRa(pvCaseCurrent.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    ...
    //上一评估时点未决赔款现金流的本期末现值_RA（用事故发生时的锁定利率曲线计息）
    measureCxUnsettled.setPvLastUlaeAmtRa(pvLastUlaeAmt.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setPvLastIbnrAmtRa(pvLastIbnrAmt.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setPvLastCaseAmtRa(pvLastCaseAmt.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    ...
    //RA部分
    measureCxUnsettled.setPaidClaimIfieRa(paidClaimIfie.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    measureCxUnsettled.setOciChangeRa(ociChange.multiply(ra).setScale(10, RoundingMode.HALF_UP));
    //如果事故年等于评估年，则为当期，否则为往期
    measureCxUnsettled.setCurrentFlag(contract.getCurrentFlag());
  ...
  public int monthsBetween(String valMonth1, String valMonth2) {
    try {
      ...
      throw new IllegalArgumentException("无法解析的日期字符串，请确保是有效的年月，格式：yyyymm", e);
    }
  }
   ...
   */
  void dealDecidedData(String valMonth,String valMethod,MeasureContext context){
    List<MeasureCxUnsettled> measureCxUnsettledList = new ArrayList<>();
    for(Map.Entry<String,MeasureCxUnsettled> entry : context.getLastUnsettledMap().entrySet()){
      //保存结果
      ...
      //上一评估时点未决赔款现金流的本期末现值_RA（用事故发生时的锁定利率曲线计息）
      measureCxUnsettled.setPvLastUlaeAmtRa(measureCxUnsettled.getUaleAmtIfieAccident().multiply(measureCxUnsettled.getRa()).setScale(10, RoundingMode.HALF_UP));
      measureCxUnsettled.setPvLastIbnrAmtRa(measureCxUnsettled.getIbnrAmtIfieAccident().multiply(measureCxUnsettled.getRa()).setScale(10, RoundingMode.HALF_UP));
      measureCxUnsettled.setPvLastCaseAmtRa(measureCxUnsettled.getCaseAmtIfieAccident().multiply(measureCxUnsettled.getRa()).setScale(10, RoundingMode.HALF_UP));
      //1. 本评估时点未决赔款现金流的本期末现值（用当期利率曲线折现）
      ...
      //本评估时点未决赔款现金流的本期末现值_RA（用当期利率曲线折现）
      measureCxUnsettled.setPvUlaeAccidentRa(BigDecimal.ZERO);
      measureCxUnsettled.setPvIbnrAccidentRa(BigDecimal.ZERO);
      measureCxUnsettled.setPvCaseAccidentRa(BigDecimal.ZERO);
      //3. 本评估时点未决赔付现金流的本期末现值（用事故发生时的锁定利率曲线折现）
      ...
      //本评估时点未决赔付现金流的本期末现值RA（用事故发生时的锁定利率曲线折现）
      measureCxUnsettled.setPvUlaeCurrentRa(BigDecimal.ZERO);
      measureCxUnsettled.setPvIbnrCurrentRa(BigDecimal.ZERO);
      measureCxUnsettled.setPvCaseCurrentRa(BigDecimal.ZERO);
      ...
      measureCxUnsettled.setServiceFeeChange(serviceFeeChange);
      measureCxUnsettled.setPaidClaimIfie(paidClaimIfie);
      measureCxUnsettled.setPaidClaimIfieRa(paidClaimIfie.multiply(measureCxUnsettled.getRa()).setScale(10, RoundingMode.HALF_UP));
      measureCxUnsettled.setOciChange(ociChange);
      measureCxUnsettled.setOciChangeRa(ociChange.multiply(measureCxUnsettled.getRa()).setScale(10, RoundingMode.HALF_UP));

      //是否转已决
      measureCxUnsettled.setDecidedFlag("1");
      String accidentYear = measureCxUnsettled.getAccidentMonth().substring(0, 4);
