...
def get_accretion_rate_factor(rates_df, start_month, end_month):
    """
    计算 CSM 计息因子 (基于起期到年底的累积利率)
    """
    if start_month >= end_month: return Decimal('0')
...
def calculate_bba_measurement():
    ...
    try:
        ...
        if rates_df.empty:
            # Fallback logic (omitted for brevity, assuming data exists or handled as before)
            print(f"⚠️ 警告: 未找到 {val_month_str} 的利率曲线。")
            return
        else:
            print(f"✅ 成功获取 {val_month_str} 利率曲线 ({len(rates_df)} 条记录)。")
        ...
        # 1.3 赔付现值
        init_fut_claim = calculate_future_pv_with_rates(actual_premium, RATIO_CLAIM, total_months, 0, rates_df)
        logger.log_item(
            "当年新增合同_初始确认_预期赔付现值",
            "初始确认时，预期未来赔付支出的折现值 (基于UEP和远期利率)",
            "SUM(Monthly Claim * Discount Factor)",
            {"Total Premium": actual_premium, "Claim Ratio": RATIO_CLAIM, "Total Months": total_months},
            init_fut_claim
        )

        # 1.4 维费现值
        init_fut_maint = calculate_future_pv_with_rates(actual_premium, RATIO_MAINT_EXP, total_months, 0, rates_df)
        logger.log_item(
            "当年新增合同_初始确认_预期维费现值",
            "初始确认时，预期未来维持费用的折现值",
            "SUM(Monthly Maint * Discount Factor)",
            {"Total Premium": actual_premium, "Maint Ratio": RATIO_MAINT_EXP},
            init_fut_maint
        )

        # 1.5 RA
        init_ra = (init_fut_claim + init_fut_maint) * RATIO_RA
        logger.log_item(
            "当年新增合同_初始确认_非金融风险调整(RA)",
            "初始确认时，对非金融风险的调整额",
            "(Claim PV + Maint PV) * RA Ratio",
            {"Claim PV": init_fut_claim, "Maint PV": init_fut_maint, "RA Ratio": RATIO_RA},
            init_ra
        )
        ...
        # 计算赔付和维费差异 (用于后续 CSM 调整，此处不展示)
        claim_var = actual_claim_incurred - expected_claim_nominal
        maint_var = actual_maint_incurred - expected_maint_nominal
        ...
        accretion_factor = get_accretion_rate_factor(rates_df, 0, months_accretion)
        ...
        # 计算总 CSM 调整额
        # 公式: Total = Prem_Var (Future) + IACF_Var (Future) - Claim_Var (Future) - Maint_Var (Future) - RA_Change
        # 用户指示: "被CSM/LC吸收的变化中才包括保费、IACF、赔付和维持、RA等变化"
        # 假设所有 Variance 都被吸收
        
        # 注意符号：Claim_Var > 0 意味着赔付增加，CSM 减少，所以是减项。
        
        exp_adj_csm_impact = prem_var - iacf_var - claim_var - maint_var - ra_change_future
        
        logger.log_item(
            "被CSM/LC吸收的变化_合计",
            "调整CSM/LC的总金额 (保费+IACF-赔付-维费-RA)",
            "Prem_Var - IACF_Var - Claim_Var - Maint_Var - RA_Change",
            {
                "Prem Var": prem_var,
                "IACF Var": iacf_var,
                "Claim Var": claim_var,
        ...
        # 剩余月数
        remaining_months_for_pv = max(0, total_months - months_passed)
        
        # PV EOP (Current Rates)
        pv_eop_claims_curr = calculate_future_pv_with_rates(actual_premium, RATIO_CLAIM, total_months, months_passed, rates_df_eop, rate_offset=0)
        pv_eop_maint_curr = calculate_future_pv_with_rates(actual_premium, RATIO_MAINT_EXP, total_months, months_passed, rates_df_eop, rate_offset=0)
        ...
        # A. PV_EOP (Locked-in)
        # 使用初始曲线 (rates_df)，但 offset = months_passed (7)
        # 剩余月数 = total_months - months_passed
        
        pv_eop_claims_lockedin = calculate_future_pv_with_rates(actual_premium, RATIO_CLAIM, total_months, months_passed, rates_df, rate_offset=months_passed)
        pv_eop_maint_lockedin = calculate_future_pv_with_rates(actual_premium, RATIO_MAINT_EXP, total_months, months_passed, rates_df, rate_offset=months_passed)
        ...
        # 计算 PV_EOP (Current Rates)
        # 使用 rates_df_eop, offset=0 (因为 EOP 曲线本身就是从 EOP 开始的)
        pv_eop_claims_current = calculate_future_pv_with_rates(actual_premium, RATIO_CLAIM, total_months, months_passed, rates_df_eop, rate_offset=0)
        pv_eop_maint_current = calculate_future_pv_with_rates(actual_premium, RATIO_MAINT_EXP, total_months, months_passed, rates_df_eop, rate_offset=0)
